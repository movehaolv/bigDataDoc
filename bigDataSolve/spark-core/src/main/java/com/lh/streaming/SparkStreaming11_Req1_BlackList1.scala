package com.lh.streaming

import java.sql.ResultSet
import java.text.SimpleDateFormat

import com.lh.util.JDBCUtil
...
object SparkStreaming11_Req1_BlackList1 {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val kafkaPara: Map[String, Object] = Map[String, Object](
            ...
            ConsumerConfig.GROUP_ID_CONFIG -> "lh",
        ...
        val kafkaDataDS: InputDStream[ConsumerRecord[String, String]] = KafkaUtils.createDirectStream[String, String](
            ...
            ConsumerStrategies.Subscribe[String, String](Set("lhNew"), kafkaPara)
        )
        ...
        ds.foreachRDD(
            rdd => {
                // rdd. foreach方法会每一条数据创建连接，val conn = JDBCUtil.getConnection 用在rdd
                // .foreach方法里会每条数据创建一个连接。连接为什么不能防在这个位置（rdd.foreach之外），因为foreach方法是RDD的算子，算子之外的代码是在Driver端执行，算子内的代码是在Executor端执行
                // 这样就会涉及闭包操作，Driver端的数据就需要传递到Executor端，需要将数据进行序列化
                // 数据库的连接对象是不能序列化的（因为數数据库访问需要授权，A被授权，可以访问B，若序列化，A发送到C服務器，C被B反序列化，則C可以访问B了，但是C并未被B授权，缺少安全性）
                // 解决办法：
                // RDD提供了一个算子可以有效提升效率 : foreachPartition
                // 可以一个分区创建一个连接对象，这样可以大幅度减少连接对象的数量，提升效率
                rdd.foreachPartition(iter => {
                        val conn = JDBCUtil.getConnection
                        iter.foreach{ // 这个foreach是集合的foreach，不是rdd的foreach，不涉及数据的序列化，可以将JDBCUtil
                            // .getConnection放在这个foreach之外

...
public class UvWithBloomFilter {
    ...
    public static class MyBloomFilter {
        ...
        public Long hashCode( String value, Integer seed ){
            ...
            System.out.println("result " + result + " " + (result & (cap - 1))  + "  " + cap);

            return result & (cap - 1); // 保留result的（cap-1)位的数据
        }
    }
    ...
    public static class UvCountResultWithBloomFliter extends ProcessAllWindowFunction<UserBehavior, PageViewCount, TimeWindow>{
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            myBloomFilter = new MyBloomFilter(1<<29);    // 要处理1亿个数据，用64MB大小的位图 (64M=pow(2,6)*pow(2,20)*pow(2,3)=pow
            // (2,29) 1向左移动29位，就是1后面有29个)
        }
